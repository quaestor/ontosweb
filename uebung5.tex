...
\begin{center}
% TODO stimmt die negation?
$ \Box(\dia p) \wedge \Box(\neg p \vee (\dia q \vee \dia r)) \wedge
\dia p \wedge \dia \neg p \wedge \Box \Box (\neg(q \vee r) \vee \neg p)
\wedge \Box (\neg\neg p \vee \Box \neg p) $

$ \neg(\Box(\dia p) \wedge \Box(\neg p \vee (\dia q \vee \dia r)) \wedge
\dia p \wedge \dia \neg p \wedge \Box \Box (\neg(q \vee r) \vee \neg p)
\wedge \Box (\neg\neg p \vee \Box \neg p)) $
...
\begin{tikzpicture}[->,>=stealth',shorten >=5pt, shorten <=5pt,auto,node distance=1.5cm,
  ...
  \node[main node,label=west:$x_21$] (2) [left of=3] {};
  \node[main node,label=west:$x_22$] (4) [right of=3] {};
...
\begin{tikzpicture}[->,>=stealth',shorten >=5pt, shorten <=5pt,auto,node distance=1.5cm,
  ...
  \node[invnode] (3) [below of=1] {};
  \node[main node,label=west:$y_21$] (2) [left of=3] {};
  \node[main node,label=west:$y_22$] (4) [right of=3] {};
  \node[bnode,label=west:$y_{31}$] (5) [below of=2] {};
  \node[cnode,label=west:$y_{32}$] (6) [below of=3] {};
  \node[cnode,label=west:$y_{33}$] (7) [below of=4] {};
